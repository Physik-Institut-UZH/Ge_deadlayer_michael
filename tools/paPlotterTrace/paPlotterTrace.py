...
TraceFiles = [
              "/Applications/adl-4.2/examples/0.50_3.48_5.00.txt",
              "/Applications/adl-4.2/examples/1.50_3.48_8.50.txt",
              "/Applications/adl-4.2/examples/2.70_3.48_8.50.txt",
              "/Applications/adl-4.2/examples/3.40_3.48_8.50.txt",
              "/Applications/adl-4.2/examples/2.70_3.48_0.50.txt",
              "/Applications/adl-4.2/examples/2.00_3.48_0.50.txt",
              "/Applications/adl-4.2/examples/1.50_3.48_0.50.txt",
              "/Applications/adl-4.2/examples/1.00_3.48_0.50.txt",
              "/Applications/adl-4.2/examples/2.70_3.48_5.00.txt"
              #"/Users/kermaidic/Documents/GERDA/ICOAX_Fields/50B_traces/0.50_3.62_1.00.txt",
              #"/Users/kermaidic/Documents/GERDA/ICOAX_Fields/50B_traces/0.50_3.62_8.00.txt"
              #"/Applications/adl-4.2/examples/ORTEC_ICPC_fields/3.70_3.78_0.05.txt",
              #"/Applications/adl-4.2/examples/ORTEC_ICPC_fields/3.50_3.78_0.05.txt",
              #"/Applications/adl-4.2/examples/ORTEC_ICPC_fields/3.30_3.78_0.05.txt",
              #"/Applications/adl-4.2/examples/ORTEC_ICPC_fields/2.90_3.78_0.05.txt",
              #"/Applications/adl-4.2/examples/ORTEC_ICPC_fields/2.70_3.78_0.05.txt"
              ]
...
if spcha_flag == 1:
  ...
  plt.imshow(pot, vmin=1e-4, vmax=1, cmap=custom_map1, origin='lower', interpolation="Nearest")#,norm=LogNorm())
elif stru_flag == 1:
	plt.imshow(pot, cmap='jet', origin='lower', interpolation="Nearest")
else:
	plt.imshow(pot, vmin=0, vmax=1200, cmap=custom_map1, origin='lower', interpolation="Nearest")

# PLOT TRACES ON TOP OF POTENTIAL
for TraceFile in TraceFiles:
	# GET TRACE DATA SAMPLE
	try:
		Pulse = np.genfromtxt(TraceFile, skip_header=1, autostrip=True)
	except IOError as e:
		print("Couldn't find the file")
		sys.exit()

	trajXe = [(CenterX-x)/(0.1*gridsize) for x in Pulse[:,0]]
	trajZe = Pulse[:,2]/(0.1*gridsize)
	trajXh = [(CenterX-x)/(0.1*gridsize) for x in Pulse[:,3]]
	trajZh = Pulse[:,5]/(0.1*gridsize)

	print("Trajectory length : ",len(trajXh),len(trajZh),len(trajXe),len(trajZe))
	plt.plot(trajXh,trajZh, 'w-',linewidth=3.0)
...
if wpot_flag == 1:
	...
	cbar.set_label('Detector component', rotation=270, fontsize=25, labelpad=20)
else:
	cbar.set_label('Electric potential (V)', rotation=270, fontsize=25, labelpad=20)
...
if spcha_flag == 1:
	...
	plt.imshow(nor, cmap=custom_map2, vmin=1e-8, vmax=0.01, origin='lower', interpolation="Nearest", norm=LogNorm())
elif stru_flag == 1:
...
# PLOT TRACES ON TOP OF POTENTIAL GRADIENT
for TraceFile in TraceFiles:
	# GET TRACE DATA SAMPLE
	try:
		Pulse = np.genfromtxt(TraceFile, skip_header=1, autostrip=True)
...
if wpot_flag == 1:
	cbar.set_label('Weighting field gradient (a.u.)', rotation=270, fontsize=25, labelpad=20)
elif stru_flag == 1:
	cbar.set_label('', rotation=270, fontsize=25, labelpad=20)
else:
