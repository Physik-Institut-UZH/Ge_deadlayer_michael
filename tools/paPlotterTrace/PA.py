...
=cut
        ...
        f = open(path, 'rb')
        header_str = f.read(4*2 + 8 + 4*4) # FIX: on failure?
        (mode, symmetry, max_voltage, nx, ny, nz, raw_mirror) \
            = struct.unpack("iidiiii", header_str)
...
=cut
        ...
        header_str = struct.pack("iidiiii", self._mode, symmetry, self._max_voltage, \
...
=cut
        """
        if mode == None: return self._mode
...
Currently, the only valid mode is -1.
Example:
...
David Manura (c) 2003-2007 Scientific Instrument Services, Inc.
Licensed under the terms of SIMION 8 or the SIMION SL Toolkit.
$Revision: 2040 $ $Date: 2007-12-08 23:52:36 -0500 (Sat, 08 Dec 2007) $ Created 2003-11.
