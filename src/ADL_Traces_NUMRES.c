...
double  ADL_G_NUMRES_DIFF_COEF  = 100.;      // cm2/s Diffusion coefficient of charge carriers in Ge
...
double GetDiffCoef(){return ADL_G_NUMRES_DIFF_COEF;}
...
int ADL_Setup_TRACES_NUMRES(char *filename_setupfile){
  ...
  if (strlen(filename_setupfile)>1) {// if a filename is supplied, parse it
    ...
    for (i=0;i<len;i++) {
      ...
      if (strcmp(Kwords[2+i]->keyword,"ADL_G_NUMRES_DIFF_COEF")==0)
        sscanf(Kwords[2+i]->svalue,"%lf",&ADL_G_NUMRES_DIFF_COEF);
    }
    free(Kwords);
  }
...
//it includes e-/holes cloud diffusion if the initial cloud extension (ADL_G_NUMRES_CLOUD_RMS) is not null
{
  ...
  int Ncloud = 16;                             // Number of cloud discretization
  
  double gridsize = GetSimionGridSize();      //predefined variable for the gridsize used
  double center = GetSimionCenter();
  double D = ADL_G_NUMRES_DIFF_COEF;          // cm2/s Diffusion coefficient of charge carriers in Ge
  ...
  double hCloudNorm;                          // -  normalize cloud densities to 1.
  double eCloudNorm;                          // -  normalize cloud densities to 1.
	    ...
	    if(ADL_G_NUMRES_XYZe[k]<0) ADL_G_NUMRES_XYZe[k]=0.0;
            ...
            if(!hCloudCenterRMS0) evnt->TD.Tr[j][k] += (evnt->HP.Eint[i])*(GetWeight(j,ADL_G_NUMRES_XYZh[k])-GetWeight(j,ADL_G_NUMRES_XYZe[k]));
              ...
              if(ADL_G_NUMRES_XYZh[k][3] >= gridsize){
                ...
                // Initialize electron cloud position in 2D plane (x,y,z) -> (r,z)
                ...
                for(int lx=0;lx<=Ncloud;lx++){
                  xhCloud[lx] = 8.*double(lx-Ncloud/2)*hCloudCenterRMS/double(Ncloud);
                  xCloudWeight[lx] = 1./(2.*pow(3.14159*D*t,1./2.)) * exp(-(pow(xhCloud[lx],2))/(2.*pow(hCloudCenterRMS,2)));
                  ...
                  for(int lz=0;lz<=Ncloud;lz++){
                    zhCloud[lz] = 8.*double(lz-Ncloud/2)*hCloudCenterRMS/double(Ncloud);
                    ...
                    if(IsInDetector(hCloudCenter)) hCloudNorm += xCloudWeight[lx]*zCloudWeight[lz];
                    if(IsInDetector(eCloudCenter)) eCloudNorm += xCloudWeight[lx]*zCloudWeight[lz];
                  }
                }
                ...
                for(int lx=0;lx<=Ncloud;lx++){
                  ...
                  for(int lz=0;lz<=Ncloud;lz++){
                    ...
                    if(IsInDetector(hCloudCenter) && hCloudNorm > 0)
                      CloudSum += zCloudWeight[lz]*xCloudWeight[lx]/hCloudNorm * GetWeight(j,hCloudCenter);
                    if(IsInDetector(eCloudCenter) && eCloudNorm > 0)
                      CloudSum -= zCloudWeight[lz]*xCloudWeight[lx]/eCloudNorm * GetWeight(j,eCloudCenter);
                  }
                }
              }
              if(CloudSum==0) evnt->TD.Tr[j][k] = evnt->TD.Tr[j][k-1];
              else evnt->TD.Tr[j][k] += (evnt->HP.Eint[i])*CloudSum;
            }
